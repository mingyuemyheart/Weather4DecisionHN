...
public class PDFView extends SurfaceView {
    ...
    public boolean swipeVertical = false;
    ...
    public void loadPages() {
    	//鑾峰彇椤甸潰瀹介珮骞惰缃〉闈㈡í绔栧睆鏄剧ず
    	if (decodeService != null) {
    		this.pageWidth = decodeService.getPageWidth(currentPage);
    		this.pageHeight = decodeService.getPageHeight(currentPage);
    		calculateOptimalWidthAndHeight();
		}
        ...
        try {
            FileUtils.fileFromLocalStorage(fileUrl, fileName, new OnFileListener() {
                ...
                public void setFile(File file) {
                            new Configurator(Uri.fromFile(file)).defaultPage(1)         //璁剧疆榛樿鏄剧ず绗�1椤�
                            .onPageChange(onPageChangeListener)     //璁剧疆缈婚〉鐩戝惉
                            .onLoad(onLoadCompleteListener)           //璁剧疆鍔犺浇鐩戝惉
                            .onDraw(onDrawListener)            //缁樺浘鐩戝惉
                            .showMinimap(false)     //pdf鏀惧ぇ鐨勬椂鍊欙紝鏄惁鍦ㄥ睆骞曠殑鍙充笂瑙掔敓鎴愬皬鍦板浘
                            .swipeVertical(swipeVertical)  //pdf鏂囨。缈婚〉鏄惁鏄瀭鐩寸炕椤碉紝榛樿鏄乏鍙虫粦鍔ㄧ炕椤�
                            .enableSwipe(true)   //鏄惁鍏佽缈婚〉锛岄粯璁ゆ槸鍏佽缈�
                            .load();
                }
            });
        } catch (IOException e) {
