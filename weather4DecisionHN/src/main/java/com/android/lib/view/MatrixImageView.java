...
/**
 * Depiction: 带放大、缩小、移动效果的ImageView
 * <p>
 * Modify:
 * <p>
 * Author: Kevin Lynn
 * <p>
 * Create Date：2015年11月25日 下午1:41:32
 * <p>
 * 
 * @version 1.0
 * @since 1.0
...
public class MatrixImageView extends ImageView {
	...
	public final MatrixTouchListener getMatrixTouchListener() {
		return mListener;
	}
	...
	public class MatrixTouchListener implements OnTouchListener {
		...
		private void checkDragable() {
			...
			//图片左边缘离开左边界，表示不可右移
			if (values[Matrix.MTRANS_X] >= 0) mRightDragable = false;
			//图片右边缘离开右边界，表示不可左移
		...
		private float checkDyBound(float[] values, float dy) {
			...
			else if (values[Matrix.MTRANS_Y] + dy < -(mImageHeight * values[Matrix.MSCALE_Y] - height)) dy = -(mImageHeight * values[Matrix.MSCALE_Y] - height) - values[Matrix.MTRANS_Y];
			return dy;
		}
		...
		private PointF getCenter(float scale, float[] values) {
			...
			if ((getWidth() / 2 - values[Matrix.MTRANS_X]) * scale < getWidth() / 2) cx = 0;
			//判断缩放后的右边缘是否会离开ImageView右边缘，是的话以右边缘为X轴中心
			if ((mImageWidth * values[Matrix.MSCALE_X] + values[Matrix.MTRANS_X]) * scale < getWidth()) cx = getWidth();
		...
		private float checkMaxScale(float scale, float[] values) {
			if (scale * values[Matrix.MSCALE_X] > mMaxScale) scale = mMaxScale / values[Matrix.MSCALE_X];
			return scale;
		}
	...
	/**
	 * Depiction: MatrixImageView移动监听接口,用以组织ViewPager对Move操作的拦截
	 * <p>
	 * Modify:
	 * <p>
	 * Author: Kevin Lynn
	 * <p>
	 * Create Date：2015年11月25日 下午2:05:32
	 * <p>
	 * 
	 * @version 1.0
	 * @since 1.0
	...
	/**
	 * Depiction: 监听ViewPager屏幕单击事件，本质是监听子控件MatrixImageView的单击事件
	 * <p>
	 * Modify:
	 * <p>
	 * Author: Kevin Lynn
	 * <p>
	 * Create Date：2015年11月25日 下午2:05:26
	 * <p>
	 * 
	 * @version 1.0
	 * @since 1.0
